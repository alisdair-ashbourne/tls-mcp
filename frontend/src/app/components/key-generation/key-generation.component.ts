...
export class KeyGenerationComponent {
  ...
  initPartiesForm(count: number) {
    ...
    for (let i = 0; i < count; i++) {
      group[`party${i}Name`] = [
        `Party ${String.fromCharCode(65 + i)}`,
        Validators.required,
      ];
      group[`party${i}Url`] = [
        `http://localhost:300${i + 1}/webhook`,
  ...
  private updatePartyValidation() {
    for (let i = 0; i < this.parties.length; i++) {
      const isParticipating = this.partiesForm.get(
        `party${i}Participate`
      )?.value;
  ...
  isPartiesFormValid(): boolean {
    ...
    for (let i = 0; i < this.parties.length; i++) {
      ...
      if (nameControl?.invalid) return false;
  ...
  async createSession() {
    if (this.sessionForm.invalid || !this.isPartiesFormValid()) return;
    ...
    try {
      ...
      const parties = this.parties.map((_, i) => {
        const isParticipating =
          participatingParty && participatingParty.partyId === i;
      ...
      const response = await firstValueFrom(
        this.apiService.createSession(sessionConfig)
      );
      ...
      if (participatingParty) {
        ...
        this.snackBar.open(
          `Session ${response.sessionId} initialized successfully!`,
          'Close',
          { duration: 3000 }
        );
      }
  ...
  private getParticipatingParty(): {
    partyId: number;
    webhookUrl: string;
  } | null {
    for (let i = 0; i < this.parties.length; i++) {
      if (!this.partiesForm.get(`party${i}Participate`)?.value) continue;
  ...
  getPartyNamePlaceholder(partyIndex: number): string {
    ...
    return isParticipating
      ? `Browser Party ${String.fromCharCode(67 + partyIndex)}`
      : 'e.g., Party A';
  }
