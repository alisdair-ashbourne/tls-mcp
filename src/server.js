  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          'Invalid request. Operation, parties, threshold, and totalParties are required.',
      });
    }
    ...
    if (threshold < 2 || threshold > totalParties) {
      return res.status(400).json({
        error:
          'Invalid threshold. Must be at least 2 and at most totalParties.',
      });
    }

    const session = await coordinatorService.initializeSession(
      operation,
      parties,
      threshold,
      totalParties,
...
app.get('/api/sessions', async (req, res) => {
  try {
    ...
    const sessions = await coordinatorService.listSessions(
      status,
      parseInt(limit) || 50
    );
...
app.post('/api/sessions/:sessionId/initiate-dkg', async (req, res) => {
  try {
    ...
    const result = await coordinatorService.initiateDistributedKeyGeneration(
      sessionId,
      blockchain
    );
...
app.post('/api/sessions/:sessionId/sign', async (req, res) => {
  try {
    ...
    console.log(
      `🔍 Calling coordinatorService.createThresholdSignature for session: ${sessionId}`
    );
    const result = await coordinatorService.createThresholdSignature(
      sessionId,
      message
    );
    console.log(
      `✅ Threshold signature creation successful for session: ${sessionId}`
    );
...
app.post('/api/sessions/:sessionId/reconstruct-key', async (req, res) => {
  try {
    ...
    console.log(`🔓 Key reconstruction requested for session: ${sessionId}`);

    const result = await coordinatorService.reconstructKeyFromShares(sessionId);

    console.log(`✅ Key reconstruction completed for session: ${sessionId}`);
...
app.post(
  '/api/sessions/:sessionId/parties/:partyId/communication-key',
  async (req, res) => {
    try {
      ...
      const result = await coordinatorService.addCommunicationPublicKey(
        sessionId,
        parseInt(partyId),
        publicKey
      );
...
app.post('/api/parties/register', async (req, res) => {
  try {
    ...
    console.log(
      `📝 Party registration received: Party ${partyId}, Wallet: ${walletAddress}`
    );
...
app.post('/api/webhook/:sessionId/:partyId', async (req, res) => {
  try {
    ...
    console.log(
      `📡 Received webhook: ${event} from Party ${partyId} for Session ${sessionId}`
    );
    ...
    console.error(
      `❌ Error handling webhook for Session ${req.params.sessionId}:`,
      error
    );
...
app.get('/api/webhook-logs/:sessionId', async (req, res) => {
  try {
    ...
    const logs = await coordinatorService.getWebhookLogs(
      sessionId,
      parseInt(limit) || 100
    );
...
app.get(
  '/api/sessions/:sessionId/parties/:partyId/webhook-events',
  async (req, res) => {
    try {
      const { sessionId, partyId } = req.params;

      const events = await coordinatorService.getPendingWebhookEvents(
        sessionId,
        parseInt(partyId)
      );
