...
const partyArg =
  args.find((arg) => arg.startsWith('--party='))?.split('=')[1] || 'a';
const portArg =
  args.find((arg) => arg.startsWith('--port='))?.split('=')[1] || '3001';
...
class PartySimulator {
  ...
  setupRoutes() {
    ...
    this.app.post('/webhook', async (req, res) => {
      try {
        ...
        console.log(
          `[Party ${PARTY_ID}] Received webhook: ${event} for session ${sessionId}`
        );
  ...
  async handleSignatureRequested(sessionId, payload) {
    console.log(
      `[Party ${PARTY_ID}] Signature requested for session ${sessionId}:`,
      payload.message
    );
  ...
  async handleReconstructionRequested(sessionId, payload) {
    console.log(
      `[Party ${PARTY_ID}] Reconstruction requested for session ${sessionId}`
    );
  ...
  async handleHeartbeat(sessionId, payload) {
    console.log(
      `[Party ${PARTY_ID}] Heartbeat received for session ${sessionId}`
    );
  ...
  async sendResponseToCoordinator(sessionId, event, payload) {
    try {
      const response = await axios.post(
        `${this.coordinatorUrl}/api/webhook/${sessionId}/${PARTY_ID}`,
        {
          event,
          payload,
          timestamp: new Date().toISOString(),
      ...
      console.error(
        `[Party ${PARTY_ID}] Error sending response to coordinator:`,
        error.message
      );
      throw error;
    }
