...
class TLSMCPDemo {
  ...
  async generateKey() {
    ...
    try {
      const response = await axios.post(`${COORDINATOR_URL}/api/sessions/${this.sessionId}/generate-key`, {
      ...
      console.error('❌ Failed to generate key:', error.response?.data || error.message);
      return false;
    }
  }
  ...
  async checkSessionStatus() {
    ...
    try {
      const response = await axios.get(`${COORDINATOR_URL}/api/sessions/${this.sessionId}`);
      ...
      console.log('   Ready parties:', session.parties.filter(p => p.status === 'ready').length);
      
      session.parties.forEach(party => {
        console.log(`   Party ${party.partyId} (${party.partyName}): ${party.status}`);
      });
  ...
  async createSignature() {
    ...
    try {
      ...
      const response = await axios.post(`${COORDINATOR_URL}/api/sessions/${this.sessionId}/sign`, {
        message: message
      });
  ...
  async reconstructKey() {
    ...
    try {
      const response = await axios.post(`${COORDINATOR_URL}/api/sessions/${this.sessionId}/reconstruct-key`);
      
      console.log('✅ Key reconstruction completed');
      console.log('💰 Wallet address:', response.data.walletAddress);
      console.log('🔑 Private key:', response.data.privateKey.substring(0, 20) + '...');
      console.log('📊 Status:', response.data.status);
      
      return true;
    } catch (error) {
      console.error('❌ Failed to reconstruct key:', error.response?.data || error.message);
      return false;
    }
  }
  ...
  async checkWebhookLogs() {
    ...
    try {
      const response = await axios.get(`${COORDINATOR_URL}/api/webhook-logs/${this.sessionId}?limit=10`);
      
      console.log(`📊 Found ${response.data.count} webhook logs`);
      
      response.data.logs.slice(0, 5).forEach(log => {
        console.log(`   ${log.direction.toUpperCase()} | Party ${log.partyId} | ${log.event} | ${log.success ? '✅' : '❌'}`);
      });
      
      return true;
    } catch (error) {
      console.error('❌ Failed to get webhook logs:', error.response?.data || error.message);
      return false;
    }
  }
